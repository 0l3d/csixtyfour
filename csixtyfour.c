...
// INSTRUCTION
struct instruction
decode_6510_opcode(uint8_t *bytes, int program_counter)
{
	struct instruction ret_val = { 0 };
	switch (bytes[program_counter]) {
	case OPCODE_NOP:
		...
		break;
	case OPCODE_CLC:
		ret_val.opcode = OPCODE_CLC;
		ret_val.bytes = 1;
		ret_val.cycles = 2;
		break;
	case OPCODE_SEC:
		ret_val.opcode = OPCODE_SEC;
		ret_val.bytes = 1;
		ret_val.cycles = 2;
		break;
	case OPCODE_LDA_IMM:
		ret_val.opcode = OPCODE_LDA_IMM;
		ret_val.bytes = 2;
		ret_val.cycles = 2;
		ret_val.imm = bytes[program_counter + 1];
		break;
	case OPCODE_ADC_IMM:
		ret_val.opcode = OPCODE_ADC_IMM;
		ret_val.bytes = 2;
		ret_val.cycles = 2;
		ret_val.imm = bytes[program_counter + 1];
		break;
	case OPCODE_SBC_IMM:
		ret_val.opcode = OPCODE_SBC_IMM;
		ret_val.bytes = 2;
		ret_val.cycles = 2;
		ret_val.imm = bytes[program_counter + 1];
		break;
...
int
run_6510_cpu(uint8_t *bytes)
{
	int             program_counter = 0;
	while (program_counter < ROM_SIZE) {
		struct instruction instrc =
			decode_6510_opcode(bytes, program_counter);
		switch (instrc.opcode) {
		case OPCODE_NOP:
			program_counter += instrc.bytes;
			machine_cycle_counter += instrc.cycles;
			break;
		case OPCODE_CLC:
		...
		case OPCODE_LDA_IMM:
			program_counter += instrc.bytes;
			machine_cycle_counter += instrc.cycles;
			accumulator = instrc.imm;
			break;
		case OPCODE_ADC_IMM:
			program_counter += instrc.bytes;
			machine_cycle_counter += instrc.cycles;
			uint16_t        result;
			result = accumulator + instrc.imm +
				(processor_status_register & 1);
			if (result > 0xFF)
				processor_status_register |= 1;
		...
		default:
			fprintf(stderr,
				"Illegal INSTRUCTION! Instruction hex: 0x%X\n",
				instrc.opcode);
			return -1;
		}
	}
...
main(int argc, char **argv)
{
	if (argv[1] == NULL) {
		fprintf(stderr,
			"Please specify a binary for emulate: %s <binary format file>",
			argv[0]);
		return 1;
	}
	struct D64_File f;
